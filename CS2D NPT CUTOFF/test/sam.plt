...
do for [i=1:nseries] {
  ...
  set ylabel 'H^*'
  set output sprintf('mc_sam%03d_ent.eps', i)
  plot sprintf('mc_sam%03d', i) using 1:3 with lines lc rgb "black"
  set ylabel 'F^*r^*'
  set output sprintf('mc_sam%03d_vir.eps', i)
  plot sprintf('mc_sam%03d', i) using 1:4 with lines lc rgb "black"
  set ylabel 'L^*'
  set output sprintf('mc_sam%03d_vol.eps', i)
  plot sprintf('mc_sam%03d', i) using 1:5 with lines lc rgb "black"
}
set xlabel 't^*'
do for [i=1:nseries] {
  set ylabel 'U^*+p^*V^*-NT^*ln(V^*)'
  ...
  set ylabel 'H^*'
  set output sprintf('md_sam%03d_ent.eps', i)
  ...
  set ylabel 'L^*'
  set output sprintf('md_sam%03d_vol.eps', i)
  plot sprintf('md_sam%03d', i) using 1:7 with lines lc rgb "black"
  set ylabel 'T^*'
  set output sprintf('md_sam%03d_tem.eps', i)
  plot sprintf('md_sam%03d', i) using 1:8 with lines lc rgb "black"
}
